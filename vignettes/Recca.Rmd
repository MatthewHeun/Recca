...
IO_list <- calc_io_mats(R = R, U = U, V = V, Y = Y, S_units = S_units)
```
...
```{r}
IO_from_list <- calc_io_mats(list(R = R, U = U, V = V, Y = Y, S_units = S_units))
class(IO_from_list)
names(IO_from_list)
IO_from_list[["y"]]
```
...
| $\eta_s$ | Energy consumed by final demand sector (rows) dividied by embodied energy input to the final demand sector  |
...
<!-- | $\colvec{\bm{\upeta}}_s$ | Energy consumed by final demand sector (rows) dividied by embodied energy input to the final demand sector  | -->
...
Figure 15 in [Heun et al. [-@Heun:2018]](https://doi.org/10.1016/j.apenergy.2018.05.109)

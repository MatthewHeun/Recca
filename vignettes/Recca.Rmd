...
The functions in `Recca` are flexible and can be used with any of 

1. matrices as inputs 
(with individual matrices as arguments to `Recca` functions),
2. a list as input (with names in the list indicating argument names), or 
3. a data frame as input (with names of data frame columns, as strings, as arguments to `Recca` functions).

Outputs are either 
named matrices in a list (for 1. and 2. above)
or
named columns appended to the input data frame (for 3. above).
The functions in `Recca` have default names for matrix arguments which follow a standard nomenclature.
...
If a particular ECC has industries whose inputs or outputs are not unit-homogeneous, 

...
[Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109).
This vignette walks through many of the calculations from [Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109),
...
(Reference [Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109), Section 2.2.2.)
...
with the `IEATools::add_psut_matnames()` and `IEATools::add_row_col_meta()` functions.
...
(Reference [Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109), Section 2.2.4.)
...
(Reference [Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109), Section 2.2.5.)
...
(Reference [Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109), Section 3.1.)
...
(Reference [Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109), Section 3.2.)
...
(Reference [Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109), Section 3.3.)
...
|---:|:---------------------------------------------------|
| $\mat{G} = \Lip \widehat{\colvec{y}}$ | Industry output requirements for final demand (i$\times$p) |
| $\mat{H} = \Lip \mat{Y}$ | Industry output requirements for final demand sectors (i$\times$s) |
| $\mat{E} = (\mat{W} + \mat{U}_{EIOU})\hatinv{\colvec{g}}$ | Energy or services produced ($+$) or consumed ($-$) per unit output by industries (in columns) (p$\times$i)|
| $\mat{M}_p$ | embodied products $\times$ embodying products |
| $\mat{M}_s$ | embodied products $\times$ consumed by final demand sectors |

`calc_embodied_mats()` also calculates $\mat{F}_{footprint}$ and $\mat{F}_{effects}$ matrices,
which answer the questions (respectively)
"What is the fractional composition of embodied energy of each final demand energy type?"
and
"What is the fractional destination of a given upstream energy product?"
The calculations can be performed for each final demand product or each final demand sector. 
The following table describes these matrices.

| Matrix | Description |
|---:|:---------------------------------------------------|
| $\mat{F}_{footprint,p}$ | Each final demand product (columns) contains embodied energy. On a fractional basis, where does that embodied energy come from (rows)? Columns sum to 1. |
| $\mat{F}_{effects,p}$ | Each upstream energy product (rows) becomes embodied somewhere. On a fractional basis, where does that upstream energy become embodied (columns)? Rows sums to 1. |
| $\mat{F}_{footprint,s}$ | Each final demand sector (columns) consumes embodied energy. On a fractional basis, where does the consumed embodied energy come from (rows)? Columns sum to 1. |
| $\mat{F}_{effects,s}$ | Each embodied upstream energy product (rows) is consumed by a final demand sector. On a fractional basis, where is that embodied upstream energy consumed (columns)? Rows sums to 1. |

Finally, embodied energy efficiencies can be calculated as final demand energy divided by embodied energy. 
Again, efficiencies can be calculated for each final demand product or each final demand sector.
The `calc_embodied_etas()` function does this computation.

| Matrix | Description |
|---:|:---------------------------------------------------|
| $\eta_p$ | Final demand product (rows) divided by embodied energy of that final demand product |
| $\eta_s$ | Energy consumed by final demand sector (rows) divided by embodied energy input to the final demand sector  |
...
Figure 15 in [Heun et al. [-@Heun:2018]](doi:10.1016/j.apenergy.2018.05.109)
shows primary-to-services energetic efficiencies for the ECC in the 3rd row of `IO_df`.
The following code extracts those results. 
Rows of the vector give final demand services and their units.
The column gives efficiencies in units of service per ktoe of energy.

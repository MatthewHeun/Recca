...
`primary_aggregates()` creates a column of primary energy sums coming from the industries specified in the `p_industries` argument.
In the example below, primary aggregates are calculated for the four rows of the `UKEnergy2000mats` data frame.

In the first example,
only 1 aggregate column is created, because there is no

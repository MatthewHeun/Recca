...
Output from the electric grid is clearly larger now, and 
so is the upstream demand for natural gas. 
...
`new_Y()` make changes upstream of final demand.
`new_k_ps()` make changes upstream of an intermediate industry.
`new_R_ps()` makes changes downstream from resource extraction.
The `new_*` functions will assist energy and exergy analyses through societies.

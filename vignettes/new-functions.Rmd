...
### Calculate the input-output structure of the ECC
...
Let's reduce `Resources - Crude` by 50 % and estimate the effect
on the rest of the ECC.

```{r}
half <- function(x){
  x/2
}
HalfCrude <- UKEnergyMatsWithR %>% 
  mutate(
    newR = elementapply_byname(half, a = R, row = "Resources - Crude", col = "Crude")
  )
HalfCrude$newR[[1]] %>% clean_byname()
```

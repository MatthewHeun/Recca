...
```{r}
library(networkD3)
make_sankey <- function(U, V, Y, simplify_edges = TRUE){
  el <- edge_list(U = U, V = V, Y = Y, simplify_edges = simplify_edges)[["Edge list"]]
  nl <- node_list(el)
  sankeyNetwork(Links = el, Nodes = nl, 
...
To do so, we must first calculat the input-output structure of the ECC 
...
k_prime <- matrix(c(1,
  ...
  setrowtype("Products") %>% setcoltype("Industries")
change_input_proportions <- new_k_ps(c(iomats, list(U = U, V = V, Y = Y, S_units = S_units, 
                                                    k_prime = k_prime)))
...
between sequantial function calls. 
...
make_sankey(U = change_final_demand$U_prime, V = change_final_demand$V_prime, 
...
```{r}
iomats2 <- calc_io_mats(U = change_input_proportions$U_prime, 
                        V = change_input_proportions$V_prime, 
                        Y = Y, S_units = S_units)
change_final_demand2 <- new_Y(c(iomats2, list(Y_prime = Y_prime)))
make_sankey(U = change_final_demand2$U_prime, 
...
But the interactions among the `new_*` fuctions can be surprising
if their behavior is not fully understood. 
This vignette provides a detailed example using the
`new_k_ps()` and `new_Y()` functions.
If they are called sequantially *without* first recalculating the input-output structure of the ECC,

...
`calc_intra_industry_balance()` calculates energy losses (waste heat)
...
To do so, 
use the function `endogenize_losses()`.
...
The $\mathbf{V}$ matrices contain a new column
...
The $\mathbf{Y}$ matrices also have a new column, 
...
In addition, 
the $\mathbf{Y}$ matrices contain a new row 
...
```{r}
endogenized2 <- intra_industry_balances |> 
...
($\mathbf{W}$ has products in rows and industries in columns while 
the intra-industry balances have industries in rows), 
* without summation by the unit vector ($\mathbf{i}$),
and 
* with opposite sign.

The following code shows the comparison
between $\mathbf{W}$ and the intra-industry balances.

...
#' #' @param no_group an identifier to be applied to \code{<NA>} entries in \code{group_colname}.
#' #'        Default is "\code{no_group}".
...
#'                        x_colname = "x",
#'                        y_colname = "y"){
#'   # Set a name for the node name columns that will be used throughout this function.
#'   node_name_colname <- ".Node_name"
#'   # The group_colname is optional.
#'   # If the column is not present, add and fill with a single group (no_group).
#'   if (!(group_colname %in% names(Industries))) {
#'     Industries <- Industries %>%
#'       dplyr::mutate(
#'         !!as.name(group_colname) := no_group
#'       )
#'   }
...
#'       )
#'   }
#'   # Eliminate factors in the incoming data frames and
#'   # fill empty group_colname observations with the no_group variable.
#'   # Industries <- Industries %>% dplyr::mutate_if(is.factor, as.character)
#'   Industries <- Industries %>%
#'     dplyr::mutate(
#'       !!group_colname := dplyr::case_when(
#'         is.na(!!as.name(group_colname)) ~ no_group,
#'         TRUE ~ !!as.name(group_colname)
#'       )
#'     ) %>%
...
#'     # Move the no_group item to the end by first deleting its row,
#'     dplyr::filter(!!as.name(group_colname) != no_group) %>%
#'     # and then adding it back.
#'     rbind(list(no_group) %>% magrittr::set_names(group_colname))

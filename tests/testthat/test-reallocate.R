testthat::test_that("reallocate_statistical_differences() works as expected", {
...
testthat::test_that("reallocate_statistical_differences() works when there are no stat diffs", {
  R <- matrix(c(100, 0,
                0, 50),
...
testthat::test_that("reallocate_statistical_differences() works with stat diffs in R only", {
  R <- matrix(c(100, 0,
                0, 48,
                0, 2),
              byrow = TRUE, nrow = 3, ncol = 2,
  ...
  expectedY <- matrix(c(20, 10,
                        0, 48), byrow = TRUE, nrow = 2, ncol = 2,
                      dimnames = list(c("Electricity", "Prod C"),
                                      c("Industry 1", "Industry 2"))) |>
    matsbyname::setrowtype("Product") |> matsbyname::setcoltype("Industry")

  # Most matrices should be unchanged
  expect_equal(res$R_prime, expectedR)
  expect_equal(res$U_prime, U)
  expect_equal(res$V_prime, V)
  expect_equal(res$Y_prime, expectedY)
  expect_equal(res$U_feed_prime, U_feed |> matsbyname::clean_byname())
